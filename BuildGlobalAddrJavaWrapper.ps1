...
$Files = @(
  ...
  [FileConfig]@{
    FileName       = "mdGlobalAddrJavaWrapper.cpp";
    ReleaseVersion = $RELEASE_VERSION;
    OS             = "ANY";
    Compiler       = "JAVA";
    Architecture   = "ANY";
...
function DownloadFiles() {
  ...
  foreach ($File in $Files) {
    ...
    if ($File.FileName -eq "mdGlobalAddr_JavaCode.zip") {
      if (!(Test-Path ("$CurrentPath\mdGlobalAddr_JavaCode.zip"))) {
        ...
        Expand-Archive -Path "$CurrentPath\mdGlobalAddr_JavaCode.zip" -DestinationPath $CurrentPath
        }
        else {
...
function CheckFiles() {
  ...
  if (!(Test-Path ("mdGlobalAddr.h"))) {
    Write-Host "mdGlobalAddr.h not found." 
    $FileMissing = $true
  }
  if (!(Test-Path ("mdGlobalAddr.lib"))) {
    Write-Host "mdGlobalAddr.lib not found." 
    $FileMissing = $true
  }
  if ($FileMissing) {
...
Write-Host "`n======================= WELCOME TO MELISSA GLOBAL ADDRESS OBJECT JAVA WRAPPER =======================`n"
...
jar cvf mdGlobalAddr.jar com\melissadata\*.class > $null

if (Test-Path ("mdGlobalAddrJavaWrapper.dll")) {
	$filePath = Join-Path -Path (Split-Path -Parent $PSCommandPath) -ChildPath "mdGlobalAddrJavaWrapper.dll"
	
	Write-Host "`nmdGlobalAddrJavaWrapper.dll has been successfully generated at:`n$filePath"
}
else {
	Write-Host "`nError: was not able to create mdGlobalAddrJavaWrapper.dll"
}

if (Test-Path ("mdGlobalAddr.jar")) {
	$filePath = Join-Path -Path (Split-Path -Parent $PSCommandPath) -ChildPath "mdGlobalAddr.jar"
	
	Write-Host "`nmdGlobalAddr.jar has been successfully generated at:`n$filePath"
}
else {
	Write-Host "`nError: was not able to create mdGlobalAddr.jar"
}
