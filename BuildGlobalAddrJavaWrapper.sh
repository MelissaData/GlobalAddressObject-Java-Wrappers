...
Config_FileName1="libmdGlobalAddr.so"
...
Config_FileName3="mdGlobalAddr.h"
...
Config_FileName4="mdGlobalAddrJavaWrapper.cpp"
...
########################## Main ############################
printf "\n======================= WELCOME TO MELISSA GLOBAL ADDRESS OBJECT JAVA WRAPPER =======================\n"
...
# Compile SO
gcc -fpic -c -I$ProjectPath -I$JavaIncludeDirectory1 -I$JavaIncludeDirectory2 mdGlobalAddrJavaWrapper.cpp
gcc -g -shared mdGlobalAddrJavaWrapper.o -o libmdGlobalAddrJavaWrapper.so -L$ProjectPath -lmdGlobalAddr -pthread

rm mdGlobalAddrJavaWrapper.o

# Compile Jar
javac com/melissadata/*.java &> /dev/null

jar cvf mdGlobalAddr.jar com/melissadata/*.class &> /dev/null

if [ -f "libmdGlobalAddrJavaWrapper.so" ];
then
	script_dir="$( cd "$( dirname "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )"
	file_path="$script_dir/libmdGlobalAddrJavaWrapper.so"
	
    printf "\nlibmdGlobalAddrJavaWrapper.so has been successfully generated at:\n$file_path\n"
else
	printf "\nError: was not able to create libmdGlobalAddrJavaWrapper.so\n"
fi

if [ -f "mdGlobalAddr.jar" ];
then
	script_dir="$( cd "$( dirname "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )"
	file_path="$script_dir/mdGlobalAddr.jar"
	
    printf "\nmdGlobalAddr.jar has been successfully generated at:\n$file_path\n"
else
	printf "\nError: was not able to create mdGlobalAddr.jar\n"
fi
